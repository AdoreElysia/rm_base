...
#define SMALL_YAW_ALIGN_ANGLE                                                                      \
    (0 * 0.043945f) // 小云台位于限制中心时，编码器值,若对云台有机械改动需要修改
#define SMALL_YAW_PITCH_HORIZON_ANGLE 0.0f // 云台处于水平位置时编码器值,若对云台有机械改动需要修改
#define SMALL_YAW_PITCH_MAX_ANGLE                                                                  \
    20.0f // 云台竖直方向最大角度 (注意反馈如果是陀螺仪，则填写陀螺仪的角度)
#define SMALL_YAW_PITCH_MIN_ANGLE                                                                  \
    -25.0f // 云台竖直方向最小角度 (注意反馈如果是陀螺仪，则填写陀螺仪的角度)

#define YAW_CHASSIS_ALIGN_ECD                                                                      \
    4088 // 云台和底盘对齐指向相同方向时的电机编码器值,若对云台有机械改动需要修改
#define YAW_ECD_GREATER_THAN_4096 0 // ALIGN_ECD值是否大于4096,是为1,否为0;用于计算云台偏转角度
#define YAW_ALIGN_ANGLE                                                                            \
    (YAW_CHASSIS_ALIGN_ECD *                                                                       \
     0.043945f) // 对齐时的角度,0-360，0.043945f =
                // (360/8192),将编码器值转化为角度制，这里指dji电机，其他电机根据手册修改
// 发射参数
