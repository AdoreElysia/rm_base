...
I2C_Device *BSP_I2C_Device_Init(I2C_Device_Init_Config *config)
{
    ...
    LOG_INFO("I2C %p device initialized successfully. Device count: %d", (void *)dev->hi2c,
             bus_manager->device_count);
    return dev;
}
...
osal_status_t BSP_I2C_Transmit(I2C_Device *dev, uint8_t *tx_data, uint16_t size)
{
    ...
    case I2C_MODE_IT:
        ...
        if (hal_status == HAL_OK)
        {
            ...
            osal_status_t wait_status =
                osal_event_wait(&bus_manager->bus_event, I2C_EVENT_TX_COMPLETE | I2C_EVENT_ERROR,
...
osal_status_t BSP_I2C_Receive(I2C_Device *dev, uint8_t *rx_data, uint16_t size)
{
    ...
    case I2C_MODE_IT:
        ...
        if (hal_status == HAL_OK)
        {
            ...
            unsigned int  actual_flags;
            osal_status_t wait_status =
                osal_event_wait(&bus_manager->bus_event, I2C_EVENT_RX_COMPLETE | I2C_EVENT_ERROR,
                                OSAL_EVENT_WAIT_FLAG_OR | OSAL_EVENT_WAIT_FLAG_CLEAR,
                                OSAL_WAIT_FOREVER, &actual_flags);
            if (wait_status != OSAL_SUCCESS)
            {
...
osal_status_t BSP_I2C_Mem_Write_Read(I2C_Device *dev, uint16_t mem_address, uint16_t mem_add_size,
                                     uint8_t *data, uint16_t size, uint8_t is_write)
{
        ...
        case I2C_MODE_IT:
            ...
            if (hal_status == HAL_OK)
            {
                ...
                osal_status_t wait_status = osal_event_wait(
                    &bus_manager->bus_event, I2C_EVENT_TX_COMPLETE | I2C_EVENT_ERROR,
    ...
    else
    {
        ...
        case I2C_MODE_IT:
            ...
            if (hal_status == HAL_OK)
            {
                ...
                unsigned int  actual_flags;
                osal_status_t wait_status = osal_event_wait(
                    &bus_manager->bus_event, I2C_EVENT_RX_COMPLETE | I2C_EVENT_ERROR,
                    OSAL_EVENT_WAIT_FLAG_OR | OSAL_EVENT_WAIT_FLAG_CLEAR, OSAL_WAIT_FOREVER,
                    &actual_flags);
                if (wait_status != OSAL_SUCCESS)
                {
