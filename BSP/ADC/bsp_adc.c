...
osal_status_t BSP_ADC_Start(ADC_Device *dev)
{
    ...
    case ADC_MODE_DMA:
        // 使用非活动缓冲区进行DMA传输
        hal_status = HAL_ADC_Start_DMA(dev->hadc, (uint32_t *)&dev->values[!dev->active_buf][0],
                                       dev->channel_count);
        break;
    ...
    case ADC_MODE_DMA:
        // 中断/DMA模式，等待事件完成
        {
            ...
            osal_status_t status =
                osal_event_wait(&dev->adc_event, ADC_EVENT_DONE,
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    for (int i = 0; i < ADC_DEVICE_NUM; i++)
    {
        if (adc_device_used[i] && adc_devices[i].hadc == hadc)
        {
            ...
            {
                HAL_ADC_Start_DMA(device->hadc, (uint32_t *)&device->values[!device->active_buf][0],
