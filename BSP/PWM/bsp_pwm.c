 ...
 * @LastEditTime: 2025-09-26 23:02:35
...
PWM_Device *BSP_PWM_Device_Init(PWM_Init_Config *config)
{
    ...
    LOG_INFO("PWM device initialized: TIM %p, Channel %d, Freq %lu Hz, Duty %u.%u%%",
             (void *)dev->htim, dev->channel, dev->frequency, dev->duty_cycle_x10 / 10,
             dev->duty_cycle_x10 % 10);
    return dev;
}
...
osal_status_t BSP_PWM_Start(PWM_Device *dev)
{
    ...
    case PWM_MODE_DMA:
        hal_status =
            HAL_TIM_PWM_Start_DMA(dev->htim, (uint32_t)dev->channel, (uint32_t *)&dev->pulse, 1);
        break;
