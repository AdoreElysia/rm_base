...
SPI_Device *BSP_SPI_Device_Init(SPI_Device_Init_Config *config)
{
    ...
    LOG_INFO("SPI %p device initialized successfully. Device count: %d", (void *)dev->hspi,
             bus_manager->device_count);
...
osal_status_t BSP_SPI_TransReceive(SPI_Device *dev, const uint8_t *tx_data, uint8_t *rx_data,
                                   uint16_t size)
{
    if (dev == NULL || tx_data == NULL || rx_data == NULL || size == 0)
    {
        LOG_ERROR("Invalid parameters: dev=%p, tx_data=%p, rx_data=%p, size=%d", (void *)dev,
    ...
    case SPI_MODE_BLOCKING:
        hal_status =
            HAL_SPI_TransmitReceive(dev->hspi, (uint8_t *)tx_data, rx_data, size, HAL_MAX_DELAY);
    ...
    case SPI_MODE_IT:
        ...
        if (hal_status == HAL_OK)
        {
            ...
            osal_status_t wait_status = osal_event_wait(
                &bus_manager->bus_event, SPI_EVENT_TX_RX_DONE_EVENT | SPI_EVENT_ERR_EVENT,
                OSAL_EVENT_WAIT_FLAG_OR | OSAL_EVENT_WAIT_FLAG_CLEAR, OSAL_WAIT_FOREVER,
                &actual_flags);
...
osal_status_t BSP_SPI_Transmit(SPI_Device *dev, const uint8_t *tx_data, uint16_t size)
{
    ...
    {
        LOG_ERROR("Invalid parameters: dev=%p, tx_data=%p, size=%d", (void *)dev, (void *)tx_data,
                  size);
        return OSAL_INVALID_PARAM;
    }
    ...
    case SPI_MODE_IT:
        ...
        if (hal_status == HAL_OK)
        {
            ...
            unsigned int  actual_flags;
            osal_status_t wait_status = osal_event_wait(
                &bus_manager->bus_event, SPI_EVENT_TX_DONE_EVENT | SPI_EVENT_ERR_EVENT,
                OSAL_EVENT_WAIT_FLAG_OR | OSAL_EVENT_WAIT_FLAG_CLEAR, OSAL_WAIT_FOREVER,
                &actual_flags);
            if (wait_status != OSAL_SUCCESS)
            {
...
osal_status_t BSP_SPI_Receive(SPI_Device *dev, uint8_t *rx_data, uint16_t size)
{
    ...
    {
        LOG_ERROR("Invalid parameters: dev=%p, rx_data=%p, size=%d", (void *)dev, (void *)rx_data,
                  size);
        return OSAL_INVALID_PARAM;
    }
    ...
    case SPI_MODE_IT:
        ...
        if (hal_status == HAL_OK)
        {
            ...
            unsigned int  actual_flags;
            osal_status_t wait_status = osal_event_wait(
                &bus_manager->bus_event, SPI_EVENT_RX_DONE_EVENT | SPI_EVENT_ERR_EVENT,
                OSAL_EVENT_WAIT_FLAG_OR | OSAL_EVENT_WAIT_FLAG_CLEAR, OSAL_WAIT_FOREVER,
                &actual_flags);
            if (wait_status != OSAL_SUCCESS)
            {
