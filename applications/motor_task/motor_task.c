...
MOTOR_DECODE_THREAD_STACK_SECTION static uint8_t
...
void motor_decode_task(ULONG thread_input)
{
    ...
    while (1)
    {
        ...
        // 等待任一设备的事件
        triggered_flag =
            BSP_CAN_ReadMultipleDevice(motor_devices, motor_device_count, OSAL_WAIT_FOREVER);
...
void motor_task_init(void)
{
    ...
    for (int i = 0; i < BENMO_MOTOR_CNT; i++)
    {
        if (benmo_motor_list[i].can_device != NULL)
        {
            ...
            if (map_count < 32)
            {
                can_event_motor_map[map_count].eventflag =
                    benmo_motor_list[i].can_device->eventflag;
...
#endif
    osal_thread_create(&motor_thread, "motor_task", motor_task, 0, &motor_thread_stack,
                       MOTOR_THREAD_STACK_SIZE, MOTOR_THREAD_PRIORITY);
    osal_thread_create(&motor_decode_thread, "motor_decode_task", motor_decode_task, 0,
                       &motor_decode_thread_stack, MOTOR_DECODE_THREAD_STACK_SIZE,
                       MOTOR_DECODE_THREAD_PRIORITY);
