 ...
 * @LastEditTime: 2025-10-27 21:21:56
...
void robot_control_init(void)
{
    ...
    if (status != OSAL_SUCCESS){LOG_ERROR("usb user init failed!");return;}
    // gimbal shoot topic 初始化
    status = itc_topic_init(&gimbal_cmd_topic, "gimbal_cmd_topic", sizeof(Gimbal_Ctrl_Cmd_s));
    if (status != OSAL_SUCCESS){LOG_ERROR("gimbal topic init failed!");return;}
    status = itc_topic_init(&shoot_cmd_topic, "shoot_cmd_topic", sizeof(Shoot_Ctrl_Cmd_s));
    if (status != OSAL_SUCCESS){LOG_ERROR("shoot topic init failed!");return;}
    // 板间通讯初始化
    status = board_com_init(&board_comm);
    if (status != OSAL_SUCCESS){LOG_ERROR("board comm init failed!");return;}
#elif defined (CHASSIS_BOARD) 
    osal_status_t status;
    // 板间通讯初始化
    status = board_com_init(&board_comm);
    ...
    if (status != OSAL_SUCCESS){LOG_ERROR("chassis topic init failed!");return;}
#else  
#error "Please define board type in robot_config.h!"
#endif 
}
...
void robot_control_task(ULONG thread_input){
    while (1) {
        ...
        // 板间通讯处理
        if (yaw_motor !=NULL){chassis_cmd.offset_angle = CalcOffsetAngle(yaw_motor->measure.angle_single_round);}else{chassis_cmd.offset_angle = 0;}
        board_com_send(&chassis_cmd);
        // 接收chassis上传数据
        status = board_com_recv(&chassis_data);
        if (status == OSAL_SUCCESS){usb_user_send(&user_send);}
        // 虚拟串口接收数据
        ...
        if (offline_module_get_device_status(board_comm.offlinemanage_index) == STATE_ONLINE)
        {
            ...
            if (status != OSAL_SUCCESS){memset(&chassis_cmd,0,sizeof(Chassis_Ctrl_Cmd_s));}
        }else
        {
...
void robot_control_task_init(void){
    osal_status_t status;
    robot_control_init();
    status = osal_thread_create(&robot_control_thread, "robot_control_thread", robot_control_task, 0,
                     robot_control_thread_stack, ROBOT_CONTROL_THREAD_STACK_SIZE, ROBOT_CONTROL_THREAD_PRIORITY);
    if (status != OSAL_SUCCESS){
        LOG_ERROR("robot_control_task_init failed!");return;
    }
