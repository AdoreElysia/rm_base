                                ...
                                unsigned int timeout_ms, osal_timer_mode_t mode)
{
    ...
    /* 创建定时器 */
    result = tx_timer_create((TX_TIMER *)timer, (CHAR *)name, (VOID(*)(ULONG))callback,
                             (ULONG)argument, timer_ticks,
                             (mode == OSAL_TIMER_MODE_PERIODIC) ? timer_ticks : 0, TX_NO_ACTIVATE);
                                ...
                                unsigned int timeout_ms, osal_timer_mode_t mode)
{
    ...
    /* 创建定时器 */
    timer->handle =
        xTimerCreateStatic(name, timer_ticks, (mode == OSAL_TIMER_MODE_PERIODIC) ? pdTRUE : pdFALSE,
