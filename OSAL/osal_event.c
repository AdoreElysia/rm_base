                              ...
                              unsigned int options, osal_tick_t timeout, unsigned int *actual_flags)
{
    ...
    result = tx_event_flags_get((TX_EVENT_FLAGS_GROUP *)event, (ULONG)requested_flags, get_option,
                                &actual_flags_local, timeout);
...
osal_status_t osal_event_set(osal_event_t *event, unsigned int flags)
{
    ...
    if (xPortIsInsideInterrupt())
    {
        ...
        result =
            xEventGroupSetBitsFromISR(event->handle, (EventBits_t)flags, &xHigherPriorityTaskWoken);
                              ...
                              unsigned int options, osal_tick_t timeout, unsigned int *actual_flags)
{
    ...
    {
        bits = xEventGroupWaitBits(event->handle, (EventBits_t)requested_flags, clear_on_exit,
                                   wait_for_all_bits, portMAX_DELAY);
    }
    else
    {
        bits = xEventGroupWaitBits(event->handle, (EventBits_t)requested_flags, clear_on_exit,
                                   wait_for_all_bits, timeout);
    }
