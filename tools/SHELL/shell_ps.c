...
void shell_ps_cmd(int argc, char **argv)
{
    if (argc < 2)
    {
        ...
        shell_module_printf(
            "Object types: thread, timer, mutex, sem, event, queue, bytepool, blockpool\r\n");
        shell_module_printf("\r\n");
        return;
    }
    ...
    if (strcmp(argv[1], "thread") == 0)
    {
        ...
        if (thread_ptr)
        {
            do
            {
                ...
                // 获取线程信息
                UINT status =
                    tx_thread_info_get(thread_ptr, &name, &state, &run_count, &priority,
                                       &preempt_threshold, &time_slice, TX_NULL, &next_thread);
                ...
                if (status == TX_SUCCESS)
                {
                    ...
                    shell_module_printf("%-32s %-8s %-10d %-12lu %-8lu\r\n", name ? name : "N/A",
                                        state_str, priority, thread_ptr->tx_thread_stack_size,
                                        run_count);
                }
            ...
            do
            {
                ...
                if (status == TX_SUCCESS)
                {
                    shell_module_printf("%-16s %-8s %-12lu\r\n", name ? name : "N/A",
                                        active ? "YES" : "NO", reschedule_ticks);
                }

                timer_ptr = next_timer;
                if (timer_ptr == TX_NULL || (ULONG)timer_ptr == TX_CLEAR_ID)
                    break;
        ...
        if (mutex_ptr && (ULONG)mutex_ptr != TX_CLEAR_ID)
        {
            do
            {
                ...
                if (status == TX_SUCCESS)
                {
                    ...
                    shell_module_printf("%-16s %-8lu %-16s\r\n", name ? name : "N/A", count_val,
                                        owner_name);
                }

                mutex_ptr = next_mutex;
                if (mutex_ptr == TX_NULL || (ULONG)mutex_ptr == TX_CLEAR_ID)
                    break;
    ...
    else if (strcmp(argv[1], "sem") == 0)
    {
        ...
        if (semaphore_ptr && (ULONG)semaphore_ptr != TX_CLEAR_ID)
        {
            do
            {
                ...
                UINT status =
                    tx_semaphore_info_get(semaphore_ptr, &name, &current_value, &first_suspended,
                                          &suspended_count, &next_semaphore);

                if (status == TX_SUCCESS)
                {
                    shell_module_printf("%-16s %-12lu %-12lu\r\n", name ? name : "N/A",
                                        current_value, suspended_count);
                }

                semaphore_ptr = next_semaphore;
                if (semaphore_ptr == TX_NULL || (ULONG)semaphore_ptr == TX_CLEAR_ID)
                    break;
    ...
    else if (strcmp(argv[1], "event") == 0)
    {
        ...
        if (event_ptr && (ULONG)event_ptr != TX_CLEAR_ID)
        {
            do
            {
                ...
                UINT status =
                    tx_event_flags_info_get(event_ptr, &name, &current_flags, &first_suspended,
                                            &suspended_count, &next_event);

                if (status == TX_SUCCESS)
                {
                    shell_module_printf("%-16s 0x%-10lx %-12lu\r\n", name ? name : "N/A",
                                        current_flags, suspended_count);
                }

                event_ptr = next_event;
                if (event_ptr == TX_NULL || (ULONG)event_ptr == TX_CLEAR_ID)
                    break;
            ...
            do
            {
                ...
                if (status == TX_SUCCESS)
                {
                    shell_module_printf("%-16s %-10lu %-10lu %-10lu\r\n", name ? name : "N/A",
                                        enqueued, available_storage, suspended_count);
                }

                queue_ptr = next_queue;
                if (queue_ptr == TX_NULL || (ULONG)queue_ptr == TX_CLEAR_ID)
                    break;
    ...
    else if (strcmp(argv[1], "bytepool") == 0)
    {
        ...
        if (byte_pool_ptr && (ULONG)byte_pool_ptr != TX_CLEAR_ID)
        {
            do
            {
                ...
                UINT status =
                    tx_byte_pool_info_get(byte_pool_ptr, &name, &available_bytes, &fragments,
                ...
                if (byte_pool_ptr == TX_NULL || (ULONG)byte_pool_ptr == TX_CLEAR_ID)
                    break;
    ...
    else if (strcmp(argv[1], "blockpool") == 0)
    {
        ...
        if (block_pool_ptr && (ULONG)block_pool_ptr != TX_CLEAR_ID)
        {
            do
            {
                ...
                UINT status =
                    tx_block_pool_info_get(block_pool_ptr, &name, &available_blocks, &total_blocks,
                                           &first_suspended, &suspended_count, &next_pool);

                if (status == TX_SUCCESS)
                {
                    shell_module_printf("%-16s %-10lu %-10lu %-10lu\r\n", name ? name : "N/A",
                                        available_blocks, total_blocks, suspended_count);
                }

                block_pool_ptr = next_pool;
                if (block_pool_ptr == TX_NULL || (ULONG)block_pool_ptr == TX_CLEAR_ID)
                    break;
    ...
    else
    {
        ...
        shell_module_printf(
            "Supported types: thread, timer, mutex, sem, event, queue, bytepool, blockpool\r\n");
        shell_module_printf("\r\n");
        return;
    }
}
