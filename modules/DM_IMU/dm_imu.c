...
{
    if (dm_imu_module == NULL || dm_imu_module->can_device == NULL || dm_imu_module->initflag != 1)
    {
        return;
    }
...
void dm_imu_update()
{
    ...
    if (dm_imu_module != NULL && dm_imu_module->initflag == 1)
    {
        ...
        {
            tmp[0] =
                dm_imu_module->can_device->rx_buff[3] << 8 | dm_imu_module->can_device->rx_buff[2];
            tmp[1] =
                dm_imu_module->can_device->rx_buff[5] << 8 | dm_imu_module->can_device->rx_buff[4];
            tmp[2] =
                dm_imu_module->can_device->rx_buff[7] << 8 | dm_imu_module->can_device->rx_buff[6];

            dm_imu_module->data.imu_data.acc[0] =
                uint_to_float(tmp[0], ACCEL_CAN_MIN, ACCEL_CAN_MAX, 16);
            dm_imu_module->data.imu_data.acc[1] =
                uint_to_float(tmp[1], ACCEL_CAN_MIN, ACCEL_CAN_MAX, 16);
            dm_imu_module->data.imu_data.acc[2] =
                uint_to_float(tmp[2], ACCEL_CAN_MIN, ACCEL_CAN_MAX, 16);
            break;
        }
        case DM_RID_GYRO:
        {
            ...
            dm_imu_module->data.imu_data.gyro[0] =
                uint_to_float(tmp[0], GYRO_CAN_MIN, GYRO_CAN_MAX, 16);
            dm_imu_module->data.imu_data.gyro[1] =
                uint_to_float(tmp[1], GYRO_CAN_MIN, GYRO_CAN_MAX, 16);
            dm_imu_module->data.imu_data.gyro[2] =
                uint_to_float(tmp[2], GYRO_CAN_MIN, GYRO_CAN_MAX, 16);
            break;
        }
        case DM_RID_EULER:
        {
            ...
            dm_imu_module->data.estimate.Pitch =
                uint_to_float(euler[0], PITCH_CAN_MIN, PITCH_CAN_MAX, 16);
            dm_imu_module->data.estimate.Yaw =
                uint_to_float(euler[1], YAW_CAN_MIN, YAW_CAN_MAX, 16);
            dm_imu_module->data.estimate.Roll =
                uint_to_float(euler[2], ROLL_CAN_MIN, ROLL_CAN_MAX, 16);

            if (dm_imu_module->data.estimate.Yaw - dm_imu_lastyaw > 180.0f)
            {
                dm_imu_module->data.estimate.YawRoundCount--;
            }
            else if (dm_imu_module->data.estimate.Yaw - dm_imu_lastyaw < -180.0f)
            {
                dm_imu_module->data.estimate.YawRoundCount++;
            }
            dm_imu_module->data.estimate.YawTotalAngle =
                360.0f * dm_imu_module->data.estimate.YawRoundCount +
                dm_imu_module->data.estimate.Yaw;
        ...
        {
            int w = dm_imu_module->can_device->rx_buff[1] << 6 |
                    ((dm_imu_module->can_device->rx_buff[2] & 0xF8) >> 2);
            int x = (dm_imu_module->can_device->rx_buff[2] & 0x03) << 12 |
                    (dm_imu_module->can_device->rx_buff[3] << 4) |
                    ((dm_imu_module->can_device->rx_buff[4] & 0xF0) >> 4);
            int y = (dm_imu_module->can_device->rx_buff[4] & 0x0F) << 10 |
                    (dm_imu_module->can_device->rx_buff[5] << 2) |
                    (dm_imu_module->can_device->rx_buff[6] & 0xC0) >> 6;
            int z = (dm_imu_module->can_device->rx_buff[6] & 0x3F) << 8 |
                    dm_imu_module->can_device->rx_buff[7];
