...
void sbus_decode(SBUS_Instance_t *sbus_instance, uint8_t *buf)
{
    ...
    if (buf == NULL || sbus_instance == NULL ||
        sbus_instance->uart_device->real_rx_len != sbus_instance->uart_device->expected_rx_len)
    {
        return;
    }
    if (buf[0] == 0x0F && buf[24] == 0x00)
    {
        ...
        // 通道数据边界检查
        if (sbus_instance->SBUS_CH.CH1 < SBUS_CHX_UP || sbus_instance->SBUS_CH.CH1 > SBUS_CHX_DOWN)
            sbus_instance->SBUS_CH.CH1 = SBUS_CHX_BIAS;
        if (sbus_instance->SBUS_CH.CH2 < SBUS_CHX_UP || sbus_instance->SBUS_CH.CH2 > SBUS_CHX_DOWN)
            sbus_instance->SBUS_CH.CH2 = SBUS_CHX_BIAS;
        if (sbus_instance->SBUS_CH.CH3 < SBUS_CHX_UP || sbus_instance->SBUS_CH.CH3 > SBUS_CHX_DOWN)
            sbus_instance->SBUS_CH.CH3 = SBUS_CHX_BIAS;
        if (sbus_instance->SBUS_CH.CH4 < SBUS_CHX_UP || sbus_instance->SBUS_CH.CH4 > SBUS_CHX_DOWN)
