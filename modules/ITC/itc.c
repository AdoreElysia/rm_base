...
osal_status_t itc_topic_init(itc_topic_t *topic, const char *topic_name, uint32_t data_len) {
    ...
    if (topic == NULL || topic_name == NULL || strlen(topic_name) >= ITC_MAX_TOPIC_NAME_LEN || data_len == 0) {
...
osal_status_t itc_subscriber_init(itc_subscriber_t *subscriber, const char *topic_name, uint32_t expected_data_len) {
    ...
    if (expected_data_len != topic->publisher_data_len) {
        LOG_ERROR("Data length mismatch for topic %s: expected %d, got %d", 
                 topic->name, topic->publisher_data_len, expected_data_len);
        osal_mutex_unlock(&itc_mutex);
        return OSAL_INVALID_PARAM;
    }
    ...
    // 创建队列用于存储数据指针
    status = osal_queue_create(&subscriber->data_queue, 
                              subscriber->name, 
                              sizeof(void*), 
                              1, 
