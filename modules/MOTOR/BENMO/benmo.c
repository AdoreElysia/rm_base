...
static const int16_t bm_mode_limits[][11] = {
    // 电机型号,
    // 开环模式最大值,开环模式最小值,电流模式最大值,电流模式最小值,电压模式最大值,电压模式最小值,速度模式最大值,速度模式最小值,位置模式最大值,位置模式最小值
...
{
    if (motor == NULL || motor->can_device == NULL)
    {
        return;
    }
    ...
    if (motor->benmo_motor_info.motor_type == P1010B_BENMO)
    {
        ...
        motor->measure.speed_rpm =
            ((int16_t)((motor->can_device->rx_buff[0] << 8) | motor->can_device->rx_buff[1])) /
            10.0f;
        motor->measure.iq =
            ((int16_t)((motor->can_device->rx_buff[2] << 8) | motor->can_device->rx_buff[3])) /
            100.0f;
        motor->measure.absolute_position =
            (uint16_t)((motor->can_device->rx_buff[4] << 8) | motor->can_device->rx_buff[5]);
        motor->measure.voltage =
            ((uint16_t)((motor->can_device->rx_buff[6] << 8) | motor->can_device->rx_buff[7])) /
            10.0f;
    }
    ...
    motor->measure.angle_single_angle =
        ECD_ANGLE_COEF_DENMO * (float)motor->measure.absolute_position; // 单圈角度
    ...
    else if (delta_ecd < -16384)
    {
        motor->measure.total_round++;
    }
    motor->measure.total_angle =
        motor->measure.total_round * 360.0f + motor->measure.angle_single_angle;
...
BenmoMotor_t *BenmoMotorInit(Motor_Init_Config_s *config, uint8_t mode_type)
{
    ...
    benmoMotor->motor_settings.control_algorithm =
    ...
    case CONTROL_PID:
        ...
        PIDInit(&benmoMotor->motor_controller.speed_PID,
                &config->controller_param_init_config.speed_PID);
        PIDInit(&benmoMotor->motor_controller.angle_PID,
                &config->controller_param_init_config.angle_PID);
        benmoMotor->motor_controller.other_angle_feedback_ptr =
            config->controller_param_init_config.other_angle_feedback_ptr;
        benmoMotor->motor_controller.other_speed_feedback_ptr =
            config->controller_param_init_config.other_speed_feedback_ptr;
        break;
    case CONTROL_LQR:
        LQRInit(&benmoMotor->motor_controller.lqr,
                &config->controller_param_init_config.lqr_config);
        benmoMotor->motor_controller.other_angle_feedback_ptr =
            config->controller_param_init_config.other_angle_feedback_ptr;
        benmoMotor->motor_controller.other_speed_feedback_ptr =
            config->controller_param_init_config.other_speed_feedback_ptr;
    ...
    {
        // 发送使能指令 (CMD=2)
        CanTxMessage_t enable_cmd = {.can_handle   = benmoMotor->can_device->can_handle,
                                     .txconf.StdId = 0x38,
                                     .txconf.IDE   = CAN_ID_STD,
                                     .txconf.RTR   = CAN_RTR_DATA,
                                     .txconf.DLC   = 8,
...
/* 修改电机的实际闭环对象 */
void BenmoMotorOuterLoop(BenmoMotor_t *motor, Closeloop_Type_e outer_loop,
...
void BenmoMotorControl(void)
{
    ...
    for (size_t i = 0; i < BENMO_MOTOR_CNT; ++i)
    {
        ...
        // 如果电机停止或离线，输出为0
        if (offline_module_get_device_status(motor->offline_index) == STATE_OFFLINE ||
            motor->stop_flag == MOTOR_STOP)
        {
            control_output = 0;
        ...
        if (mode_limits_index != -1)
        {
            ...
            {
            case BM_OPEN_MODE:
                min_val = bm_mode_limits[mode_limits_index][1];
                max_val = bm_mode_limits[mode_limits_index][2];
                break;
            case BM_CUR_MODE:
                min_val = bm_mode_limits[mode_limits_index][3];
                max_val = bm_mode_limits[mode_limits_index][4];
                break;
            case BM_VOL_MODE:
                min_val = bm_mode_limits[mode_limits_index][5];
                max_val = bm_mode_limits[mode_limits_index][6];
                break;
            case BM_SPD_MODE:
                min_val = bm_mode_limits[mode_limits_index][7];
                max_val = bm_mode_limits[mode_limits_index][8];
                break;
            case BM_POS_MODE:
                min_val = bm_mode_limits[mode_limits_index][9];
                max_val = bm_mode_limits[mode_limits_index][10];
                break;
            default:
                min_val = 0;
                max_val = 0;
        ...
        if (motor->benmo_motor_info.motor_type == P1010B_BENMO)
        {
            ...
            {
            case BM_CUR_MODE:
                send_value = (int16_t)(control_output * 100.0f);
                break; // 电流: A*100
            case BM_VOL_MODE:
                send_value = (int16_t)(control_output * 100.0f);
                break; // 电压: V*100
            case BM_SPD_MODE:
                send_value = (int16_t)(control_output * 10.0f);
                break; // 速度: RPM*10
            case BM_POS_MODE:
                send_value = (int16_t)(control_output * 100.0f);
                break; // 位置: 圈*100
            default:
                send_value = (int16_t)control_output;
                break;
            }
        }
        else if (motor->benmo_motor_info.motor_type == M1505A_BENMO)
        {
            ...
            {
            case BM_CUR_MODE:
                send_value = (int16_t)((control_output / 55.0f) * 32767.0f);
                break; // 电流归一化
            case BM_VOL_MODE:
                send_value = (int16_t)control_output;
                break; // 电压直接发送 (假设control_output已经是协议值)
            case BM_SPD_MODE:
                send_value = (int16_t)(control_output * 10.0f);
                break; // 速度: RPM*10
            default:
