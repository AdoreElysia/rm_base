...
osal_status_t beep_init(uint32_t frequency, uint32_t beep_time_period,
                        osal_timer_callback_t callback)
{
    osal_status_t status = OSAL_SUCCESS;
    // 配置PWM设备
    PWM_Init_Config pwm_config = {.htim           = &htim4,
    ...
    status = osal_timer_create(&beep_timer, "beep_timer", beep_callback, NULL, beep_time_period,
