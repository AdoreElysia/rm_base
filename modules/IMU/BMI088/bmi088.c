...
/* 内部使用的bmi088读写函数 */
static inline osal_status_t _bmi088_writedata(SPI_Device *device, const uint8_t addr,
...
static inline osal_status_t _bmi088_readdata(SPI_Device *device, const uint8_t addr, uint8_t *data,
                                             const uint8_t data_len, uint8_t isacc)
{
    osal_status_t status = OSAL_SUCCESS;
...
static inline void bmi088_gyro_init(BMI088_Instance_t *ist)
{
    ...
    for (uint8_t i = 0; i < sizeof(BMI088_Gyro_Init_Table) / sizeof(BMI088_Gyro_Init_Table[0]); i++)
    {
        ...
        DWT_Delay(0.001);
        _bmi088_readdata(ist->gyro_device, reg, &tmp, 1,
                         0); // 写完之后立刻读回对应寄存器检查是否写入成功
        DWT_Delay(0.001);
...
osal_status_t bmi088_get_accel(BMI088_Instance_t *ist, IMU_Data_t *IMU_Data)
{
    ...
    for (uint8_t i = 0; i < 3; i++)
    {
        ...
        {
            IMU_Data->acc[i] = (BMI088_ACCEL_6G_SEN) *
                               (float)((int16_t)((ist->buf[2 * i + 1]) << 8) | ist->buf[2 * i]) *
                               ist->BMI088_Cali_Offset.AccelScale;
        }
        else
        {
            IMU_Data->acc[i] = (BMI088_ACCEL_6G_SEN) *
...
osal_status_t bmi088_get_temp(BMI088_Instance_t *ist, IMU_Data_t *IMU_Data)
{
    ...
    osal_status_t status = OSAL_SUCCESS;
    status               = _bmi088_readdata(ist->acc_device, BMI088_TEMP_M, ist->buf, 2,
                                            1); // 读温度,温度传感器在accel上
...
osal_status_t bmi088_get_gyro(BMI088_Instance_t *ist, IMU_Data_t *IMU_Data)
{
    ...
    osal_status_t status = OSAL_SUCCESS;
    status               = _bmi088_readdata(ist->gyro_device, BMI088_GYRO_X_L, ist->buf, 6,
                                            0); // 连续读取3个(3*2=6)轴的角速度
    if (ist->BMI088_Cali_Offset.Calibrated)
    {
        for (uint8_t i = 0; i < 3; i++)
        {
            IMU_Data->gyro[i] =
                BMI088_GYRO_2000_SEN *
                    (float)((int16_t)((ist->buf[2 * i + 1]) << 8) | ist->buf[2 * i]) -
                ist->BMI088_Cali_Offset.GyroOffset[i];
        }
    }
    else
    {
        for (uint8_t i = 0; i < 3; i++)
        {
            IMU_Data->gyro[i] = BMI088_GYRO_2000_SEN *
                                (float)((int16_t)((ist->buf[2 * i + 1]) << 8) | ist->buf[2 * i]);
        }
    }
    return status;
...
osal_status_t BMI088_init(BMI088_Instance_t *ist)
{
    ...
    static PWM_Init_Config config = {.channel        = PWM_CHANNEL_1,
                                     .duty_cycle_x10 = 0,
                                     .frequency      = 5000,
                                     .htim           = &htim10,
